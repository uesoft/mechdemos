...
%                                                                         %
%     Authors(s) G. D. Wood, J. Wendlandt, J. Glass, D. C. Kennedy        %
%                                                                         %
...
%  gap metric (2).                                                        %                                                                         %
%                                                                         %    
%  Special commands in the Control System and Robust Control Toolboxes.   %
%                                                                         %
%  (1) K. Glover and D. C. McFarlane. Robust stabilization of normalised  %
%  coprime factor plant descriptions with H_infinity bounded              %
...
% Path check for Control System Toolbox
if isempty(findstr('control',path))
    disp('Control System Toolbox is not installed on your path.');
    return
end
...
% Path check for Robust Control Toolbox
if isempty(findstr('robust',path))
    disp('Robust Control Toolbox is not installed on your path.');
    return
end
...
ylabel('log controller frequency response')

[sysk,emax,sysobs] = ncfsyn(Pw,1.1,'ref'); % Synthesize an optimal loopshaping controller.
